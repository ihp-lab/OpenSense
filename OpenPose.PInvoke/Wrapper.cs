...
namespace OpenSense.Components.OpenPose.PInvoke {
	...
	public class Wrapper {
		...
		#region Properties
		// OpenPose state
		public static OPState State { get; private set; } = OPState.Ready;

		// Output
		private static Datum currentData;
		private static Queue<Datum> dataBuffer = new Queue<Datum>();
		...
		public static void OPEnableImageOutput(bool enable) {
			Bind._OPSetImageOutputEnable(enable);
		}
		...
		public static bool OPGetOutput(out Datum data) {
		...
		public static void OPConfigureExtra(
			...
			Bind._OPConfigureExtra(reconstruct3d, minViews3d, identification, tracking, ikThreads);
		}
		...
		// Log callback
		private static Bind.DebugCallback OPLog = delegate (string message, int type) {
			switch (type) {
		...
		// Output callback
		private static Bind.OutputCallback OPOutput = delegate (IntPtr ptrPtr, int ptrSize, IntPtr sizePtr, int sizeSize, byte outputType) {
			...
			OutputParser.ParseOutput(ref currentData, ptrArray, sizeArray, (OutputType)outputType);
		};
		#endregion
