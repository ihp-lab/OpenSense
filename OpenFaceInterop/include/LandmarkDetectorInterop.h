...
namespace OpenFaceInterop
{
	...
	public:
		...
		FaceModelParameters(System::String^ root, bool ceclm, bool clnf, bool clm)
		{
			...
			std::string model_loc = "models/main_ceclm_general.txt";
			if (ceclm)
			{
				model_loc = "models/main_ceclm_general.txt";
			}
			else if(clnf)
			{
				model_loc = "models/main_clnf_general.txt";
			}
			else if (clm)
			{
				model_loc = "models/main_clm_general.txt";
	...
	public:
		...
		bool DetectLandmarksInVideo(RawImage^ rgb_image, FaceModelParameters^ modelParams, RawImage^ gray_image) {
			return ::LandmarkDetector::DetectLandmarksInVideo(rgb_image->Mat, *clnf, *modelParams->getParams(), gray_image->Mat);
		}

		bool DetectFaceLandmarksInImage(RawImage^ rgb_image, FaceModelParameters^ modelParams, RawImage^ gray_image) {
			return ::LandmarkDetector::DetectLandmarksInImage(rgb_image->Mat, *clnf, *modelParams->getParams(), gray_image->Mat);
		}
			
		bool DetectFaceLandmarksInImage(RawImage^ rgb_image, Rect^ bounding_box, FaceModelParameters^ modelParams, RawImage^ gray_image) {
