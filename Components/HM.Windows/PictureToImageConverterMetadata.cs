...
namespace OpenSense.Components.HM {
    ...
    public sealed class PictureToImageConverterMetadata : ConventionalComponentMetadata {
        ...
        public override string Description => "[Experimental] Convert HM PictureSnapshot to \\psi Image. Supports chroma conversion, bit depth mapping, and multiple output pixel formats.";

        protected override Type ComponentType => typeof(PictureToImageConverter);

        protected override string? GetPortDescription(string portName) {
            return portName switch {
                nameof(PictureToImageConverter.In) => "The input PictureSnapshot stream.",
