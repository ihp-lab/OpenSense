...
namespace OpenSense.Components.OpenSmile {
    ...
    public class OpenSmile : INotifyPropertyChanged {
        ...
        public ImmutableDictionary<string, Emitter<Vector>> Out { private set; get; } = ImmutableDictionary<string, Emitter<Vector>>.Empty;
        ...
        public OpenSmile(Pipeline pipeline, Configuration config) {
#if !DEBUG
            ...
            foreach (var name in env.RawDataSinkInstanceNames()) {
                var receiver = pipeline.CreateEmitter<Vector>(this, name);
        ...
        protected RawDataHandler CreateRawDataHandler(string sinkName) {
            return (data) => {
                try {
                    var time = StartTime + TimeSpan.FromSeconds(data.Time);//TODO: examine time
                    Out[sinkName].Post(new Vector(data), time.DateTime);
                } catch (Exception ex) {
