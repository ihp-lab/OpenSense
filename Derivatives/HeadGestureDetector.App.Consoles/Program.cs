            ...
            try {
                ...
                foreach (string line in File.ReadLines(options.Input)) {
                    ...
                    if (lineIndex == 0) {
                         ...
                         */
                        spliter = Regex.Matches(line, ", *").Select(m => m.Value).Distinct().Single();
                        var columns = line.Split(spliter);
                        ...
                        outputFile.Value.WriteLine(line + $"{spliter}gesture");
                    } else {
                        /* Data Line */
                        Trace.Assert(spliter is not null);
                        ...
                        if (!success) {
                            outputFile.Value.WriteLine($"{line}{spliter}null");
                        } else {
                            ...
                            outputFile.Value.WriteLine($"{line}{spliter}{value}");
                        }
                    }
                    lineIndex++;
                }
