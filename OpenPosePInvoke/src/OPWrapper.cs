...
namespace OpenPose {
	...
	public class OPWrapper {
		...
		// Output
		private static OpenPoseDatum currentData;
		private static Queue<OpenPoseDatum> dataBuffer = new Queue<OpenPoseDatum>();
		...
		public static bool OPGetOutput(out OpenPoseDatum data) {
			if (dataBuffer.Count > 0) {
				data = dataBuffer.Dequeue();
				return true;
			} else {
				data = new OpenPoseDatum();
				return false;
			}
		}
