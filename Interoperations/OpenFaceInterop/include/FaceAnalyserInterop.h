...
namespace OpenFaceInterop
{
	...
	public:
		...
		FaceAnalyser(System::String^ root, bool dynamic, int output_width, bool mask_aligned)
		{
		...
		void AddNextFrame(RawImage^ frame, List<System::Tuple<float, float>^>^ landmarks, bool success, bool online) {
		...
		System::Tuple<Dictionary<System::String^, double>^, Dictionary<System::String^, double>^>^
			PredictStaticAUsAndComputeFeatures(RawImage^ frame, List<System::Tuple<float, float>^>^ landmarks)
		{
		...
		RawImage^ GetLatestHOGFeature() {
			RawImage^ HOG_feature = gcnew RawImage(*hog_features);
			return HOG_feature;
		}
