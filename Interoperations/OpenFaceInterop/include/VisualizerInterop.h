...
namespace OpenFaceInterop
{
	...
	public:
		...
		void SetObservationFaceAlign(RawImage^ aligned_face_image)
		{
			m_visualizer->SetObservationFaceAlign(aligned_face_image->Mat);
		}

		void SetObservationHOG(RawImage^ observation_HOG, int num_cols, int num_rows)
		{
		...
		void SetImage(RawImage^ canvas, float fx, float fy, float cx, float cy)
		{
		...
		RawImage^ GetHOGVis()
		{
			RawImage^ hog_image = gcnew RawImage(m_visualizer->GetHOGVis());
			return hog_image;
		}

		RawImage^ GetVisImage()
		{
			RawImage^ vis_image = gcnew RawImage(m_visualizer->GetVisImage());
			return vis_image;
		}
