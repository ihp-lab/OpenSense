...
namespace OpenFaceInterop
{
	...
	private:
		...
		RawImage^ m_rgb_frame;
		RawImage^ m_gray_frame;
	...
	public:
		...
		RawImage^ GetNextImage()
		{
			cv::Mat next_image = m_sequence_capture->GetNextFrame();
			m_rgb_frame = gcnew RawImage(next_image);
		...
		RawImage^ GetCurrentFrameGray() {
			...
			m_gray_frame = gcnew RawImage(next_gray_image);
	...
	public:
		static List<System::Tuple<int, System::String^, List<System::Tuple<int, int>^>^, RawImage^>^>^ GetCameras(System::String^ root_directory_m)
		{
			auto managed_camera_list = gcnew List<System::Tuple<int, System::String^, List<System::Tuple<int, int>^>^, RawImage^>^>();
			...
			for (size_t i = 0; i < num_cameras; ++i)
			{
				...
				RawImage^ sample_img_managed = gcnew RawImage(sample_img);
				managed_camera_list->Add(gcnew System::Tuple<int, System::String^, List<System::Tuple<int, int>^>^, RawImage^>(i, device_name_m, resolutions, sample_img_managed));
			}
